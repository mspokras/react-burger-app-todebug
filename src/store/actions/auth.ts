...
export const authSuccess = (token: $TSFixMe, userId: $TSFixMe) => {
  return {
    type: AUTH_SUCCESS,
    idToken: token,
    userId
...
export const authFail = (error: $TSFixMe) => {
  return {
    type: AUTH_FAIL,
    error
  };
...
export const checkAuthTimeout = (expirationTime: $TSFixMe) => {
  return {
...
export const auth = (email: $TSFixMe, password: $TSFixMe, isSignup: $TSFixMe) => {
  // these actions are executed in auth sagas
  // return dispatch => {
  //   dispatch(authStart());
  //   const authData = {
...
export const setAuthRedirectPath = (path: $TSFixMe) => {
  return {
